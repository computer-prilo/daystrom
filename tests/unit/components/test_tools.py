...
class TestWebFetch:
    ...
    def test_fetch_markdown_plain_text(self, mocker, mock_response):
        ...
        result = web_fetch("https://example.com", format="markdown")
    ...
    def test_fetch_plain_text(self, mocker, mock_response):
    ...
    def test_fetch_markdown_converts_html(self, mocker, mock_response):
    ...
    def test_fetch_text_converts_html(self, mocker, mock_response):
    ...
    def test_fetch_html_returns_raw(self, mocker, mock_response):
    ...
    def test_fetch_json_returns_raw_text(self, mocker, mock_response):
        ...
        result = web_fetch("https://example.com", format="json")

        assert result == json_text
        call_kwargs = mock_get.call_args.kwargs
        assert "application/json" in call_kwargs["headers"]["accept"]

    def test_fetch_json_parses_json_content_type(self, mocker, mock_response):
    ...
    def test_default_format_is_markdown(self, mocker, mock_response):
    ...
    def test_unsupported_format_raises_error(self, mocker, mock_response):
        """Test that unsupported format raises ToolCallError."""
        response = mock_response(text="Content")
        mocker.patch("httpx.get", return_value=response)

        with pytest.raises(ToolCallError):
            web_fetch("https://example.com", format="invalid_format")

    def test_http_error_propagates(self, mocker, mock_response):
    ...
    def test_timeout_is_set(self, mocker, mock_response):
    ...
    def test_url_is_passed_correctly(self, mocker, mock_response):
        """Test that URL is passed correctly to httpx.get."""
        response = mock_response(text="Content")
        mock_get = mocker.patch("httpx.get", return_value=response)

        web_fetch("https://example.com/path?query=value")

        mock_get.assert_called_once()
        call_args = mock_get.call_args
        assert call_args[0][0] == "https://example.com/path?query=value"

    def test_accept_header_markdown_format(self, mocker, mock_response):
    ...
    def test_accept_header_text_format(self, mocker, mock_response):
        """Test accept header priorities for text format."""
        response = mock_response(text="Content")
        mock_get = mocker.patch("httpx.get", return_value=response)

        web_fetch("https://example.com", format="text")

        accept = mock_get.call_args.kwargs["headers"]["accept"]
        # plain text should have highest priority
        assert "text/plain;q=1.0" in accept

    def test_accept_header_html_format(self, mocker, mock_response):
        """Test accept header priorities for html format."""
        response = mock_response(text="Content")
        mock_get = mocker.patch("httpx.get", return_value=response)

        web_fetch("https://example.com", format="html")

        accept = mock_get.call_args.kwargs["headers"]["accept"]
        # html should have highest priority
        assert "text/html;q=1.0" in accept

    def test_accept_header_json_format(self, mocker, mock_response):
        """Test accept header priorities for json format."""
        response = mock_response(text="Content")
        mock_get = mocker.patch("httpx.get", return_value=response)

        web_fetch("https://example.com", format="json")
